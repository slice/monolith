module.exports = class Scanner {
  ...
  peek() {
    if (this.index + 1 === this.maxIndex) return null;
  ...
  arg() {
    ...
    if (word.startsWith('"')) {
      ...
      let phrase = this.gatherUntil('"');
      // skip spaces after a quoted phrase
      if (!this.atEnd && this.current === ' ') this.index++;
      return phrase;
    } else {
      return word;
    }
  ...
  gatherUntil(_predicate, { skipAfter = true } = {}) {
    let predicate = typeof _predicate === 'string'
